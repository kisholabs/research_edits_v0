...
\begin{figure}[!htb]
   \begin{minipage}{0.33\textwidth}
     \centering
     \includegraphics[width=.9\linewidth]{figures/LASSO_Trade_Off_100_Zoomed_In_USING.png}
     \caption{(a)}\label{Fig:Data1a}
   \end{minipage}\hfill
   \begin{minipage}{0.33\textwidth}
     \centering
     \includegraphics[width=.9\linewidth]{figures/Ridge_Trade_Off_USING.png}
     \caption{(b)}\label{Fig:Data1b}
   \end{minipage}
   \begin{minipage}{0.33\textwidth}
     \begin{tabular}{l|l|l}
...
Both the Ridge and LASSO regression models achieved a near identical fit to the velocity field, achieving $R^{2}$ values of 0.999 and 0.985 respectively. The model predicted plotted against the ground truth velocity field has been shown in Figure \ref{Fig:Data4}, highlighting the accuracy of the predictions. The slight errors in the predictions seem to be mostly located along the coastline, and we believe that these errors can be corrected for through a smaller grid cell size and access to higher computing power. 
...
Future Work is getting a better fit

%The findings are an uneven distribution of basal tractions which is important because 

%How do we go from stress terms towards friction coefficent. Do we have time to do that, otherwise its a force balance problem and the distributions of the stresses at the base that are needed to counteract gravity.

%We need the relationship between ice flow velocity, ice flow thickness, and friction, and in doing that we need to find tau xz (sheer strain times the viscosity), the sheer strain involves spatial derivatives. There are published relationships for friction of glaciers. We can do an conversion of our traction results to friction using Tau xz
