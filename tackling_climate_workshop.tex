...
Both the Ridge and LASSO regression models achieved a near identical fit to the velocity field, achieving $R^{2}$ values of 0.999 and 0.985 respectively. The model predictions (green vectors) have been plotted against the ground truth velocity field (red vectors) in Figure \ref{Fig:Data4}, highlighting the accuracy of the predictions. Given the unusually identical fit, we are planning on running this model on larger datasets to verify this accuracy. We can now take the model's coefficients and convert our strain rate basis functions to basal tractions through the method described in  BLANK \cite{holt}. The basal tractions are plotted in \ref{Fig:Data5}, and indicate that the basal tractions responsible for holdnig the GrIs together mostly lie on the coastline of the ice sheet.
